...
本節は **`phase0-perf` をマージした `2026-04-04-mq2r` 上**の `scripts/inventory_manager_v3.py` で、純之介が実施した **dry-run** の記録である。本番 cron 再有効化・本番 deploy は本タスク範囲外。

### 8.1 20 件 dry-run（`--limit 20`）

| 項目 | 値 |
|------|-----|
| 開始 | 2026-04-23 15:10:12 |
| 終了 | 2026-04-23 15:12:49 |
| 所要 | **156 秒**（約 2 分 36 秒） |
| 1 件あたり（平均） | 約 **7.8 秒** |
| OOS 誤判定 | **0** |
| Playwright タイムアウト | **0** |
...
### 8.2 108 件 dry-run（全件・`--limit` なし）

| 項目 | 値 |
|------|-----|
| 開始 | 2026-04-23 15:18:05 |
| 終了 | 2026-04-23 15:31:04 |
| 所要 | **778 秒**（約 **13 分**） |
| 1 件あたり（平均） | 約 **7.2 秒** |
| OOS 誤判定 | **0** |
| Playwright タイムアウト | **0** |
| 二系統不一致 | **0** |
| 備考 | 全件 **checkout-button 検出**で Active 維持 |

**目標**（111 件程度を 30 分以内）を大きく下回って完走。Phase 0 の安全性（リトライ・二系統・曖昧時は OOS しない）は実測上も維持された。

### 8.3 改善前（2026-04-22 深夜）との比較

| 版 | 条件 | 結果の目安 |
|----|------|------------|
| **改善前** | `networkidle` + 180s 下限 + 固定 10s 等（§2.1） | 約 **16〜25 分**経過時点で **111 件中ほぼ進まず**（CPU ほぼ待機・実用外） |
| **改善後** | §4 のパッチ適用後 dry-run | **108 件が約 13 分**、タイムアウト・二系統不一致・誤 OOS は **いずれも 0** |

倍率の目安: 同規模を **おおよそ 1/10 以下の壁時計**で巡回可能になった（ネットワーク・メルカリ負荷により変動）。
